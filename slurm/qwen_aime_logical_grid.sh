#!/bin/bash
# Qwen-7B pooled AIME (2024/2025/2026) under LOGICAL positions on a 4xH100
# box, two cells per card. Same cell layout as aime_logical_grid.sh; bands
# 20/24 for Qwen. Pool the year files with scripts/pool_aime.py.
set -u
...
python3 - <<PREFLIGHT || { echo "[grid] ABORT: need $NGPU CUDA devices"; exit 1; }
import sys, torch
ok = torch.cuda.is_available() and torch.cuda.device_count() >= $NGPU
