...
def main():
    ...
    ax.text(sorted(none)[-1], none[sorted(none)[-1]]["tokens_per_s"] * 1.12, "no eviction", color=fs.GREY,
            fontsize=7, ha="right", va="bottom")
    ax.text(sorted(seg)[-1], seg[sorted(seg)[-1]]["tokens_per_s"] * 0.8, "EpiKV-Seg, $K$=1024", color=fs.BLUE,
    ...
    bx.set_xticks(Bs)
    bx.set_xticklabels([str(b) for b in Bs])
